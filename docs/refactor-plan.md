...
- 1C.2: 上传后用 magic bytes 校验真实文件类型，文件名改用 crypto.randomUUID
- 1C.3: Double-Submit Cookie 模式 CSRF 防护，前端 axios 拦截器自动附加 x-csrf-token
- 验证：后端模块全部加载成功，前端 `npm run build` 构建通过
