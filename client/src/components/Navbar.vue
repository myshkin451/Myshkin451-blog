<template>
  <nav
    class="sticky top-0 z-50 w-full border-b border-zinc-200/70 bg-white/65 backdrop-blur dark:border-zinc-800/70 dark:bg-zinc-950/60"
  >
    <!-- scroll progress -->
    ...
    <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
      ...
      <router-link to="/" class="group flex items-center gap-3">
        <div
          class="grid h-9 w-9 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-900 shadow-sm transition-colors group-hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50 dark:group-hover:bg-zinc-900/40"
        >
      ...
      <div class="hidden md:flex items-center gap-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            class="grid h-9 w-9 place-items-center rounded-full border border-zinc-200 bg-white/60 text-zinc-700 transition-colors hover:bg-white dark:border-zinc-800 dark:bg-zinc-900/40 dark:text-zinc-200 dark:hover:bg-zinc-900/70"
          ...
          <div v-if="isLoggedIn" ref="profileMenuRef" class="relative">
            <button
              class="flex items-center gap-2 rounded-full border border-zinc-200 bg-white/60 pl-1 pr-3 py-1.5 transition-colors hover:bg-white dark:border-zinc-800 dark:bg-zinc-900/40 dark:hover:bg-zinc-900/70"
            ...
            <div
              ...
              class="absolute right-0 mt-2 w-56 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-950"
            >
              ...
              <button
                class="menu-item w-full text-left text-red-600 dark:text-red-400"
                @click="logout"
              >
                退出登录
              </button>
          ...
          <div v-else class="flex items-center gap-2">
            ...
            <router-link
              to="/register"
              class="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-sm text-zinc-900 shadow-sm transition-colors hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50 dark:hover:bg-zinc-900"
            >
              注册
            </router-link>
          </div>
      ...
      <!-- Mobile toggle -->
      <button
        class="md:hidden grid h-10 w-10 place-items-center rounded-full border border-zinc-200 bg-white/60 text-zinc-700 transition-colors hover:bg-white dark:border-zinc-800 dark:bg-zinc-900/40 dark:text-zinc-200 dark:hover:bg-zinc-900/70"
    ...
    >
      <div class="mx-auto max-w-6xl px-6 py-4 space-y-4">
        ...
        <div class="flex flex-col gap-1">
          <router-link to="/" :class="mobileLinkClass('/')" @click="closeMobileMenu"
            >首页</router-link
          >
          <router-link
            to="/categories"
            :class="mobileLinkClass('/categories')"
            @click="closeMobileMenu"
            >分类</router-link
          >
          <router-link to="/tags" :class="mobileLinkClass('/tags')" @click="closeMobileMenu"
            >标签</router-link
          >
          <router-link
            to="/guestbook"
            :class="mobileLinkClass('/guestbook')"
            @click="closeMobileMenu"
            >留言板</router-link
          >
        ...
        <div
          class="flex items-center justify-between pt-2 border-t border-zinc-200/70 dark:border-zinc-800/70"
        >
          <button
            class="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-800 transition-colors hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:bg-zinc-900"
            @click="toggleTheme"
          >
          ...
          <button v-else class="text-sm font-medium text-red-600 dark:text-red-400" @click="logout">
            退出登录
          </button>
        </div>
      </div>
